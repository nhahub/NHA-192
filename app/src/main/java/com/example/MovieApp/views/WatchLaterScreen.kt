...
fun WatchLaterScreen(viewModel: MoviesViewModel) {
    ...
    val watchLaterMovies by viewModel.WatchLaterMovies.collectAsState(initial = emptyList())

    Scaffold(
        topBar = { WatchLaterTopBar() },
        modifier = Modifier
            .fillMaxSize()
        ...
        Box() {
            ...
            if(watchLaterMovies.isEmpty()) {
                Box(
                    ...
                    Text("暂无收藏 No Movies to be watched later yet")
                }
            }
            else{
                LazyVerticalGrid(
